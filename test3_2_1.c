...
int main(int argc,char** argv)
{
    ...
    if(id == 0){    //read file of file 1
        ...
        for(i = 1 ;i <p ; i++){
            ...
            MPI_Send(&matA[i*slave_data],    slave_data, MPI_DOUBLE,   i, 3, MPI_COMM_WORLD);
            MPI_Send(&matB[i*slave_data],    slave_data, MPI_DOUBLE,   i, 4, MPI_COMM_WORLD);
            MPI_Send(&matAB[i*slave_data],    slave_data, MPI_DOUBLE,   i, 5, MPI_COMM_WORLD);
            MPI_Irecv(&matAB[i*slave_data],    slave_data, MPI_DOUBLE,   i, 6, MPI_COMM_WORLD,&request[0]);
        ...
        MPI_Irecv(&slave_loc,     1, MPI_INT,     0, 1, MPI_COMM_WORLD,&request[1]);
        MPI_Irecv(&slave_data,    1, MPI_INT,     0, 2, MPI_COMM_WORLD, &request[2]);
        ...
        MPI_Irecv(matA_recv,  slave_data, MPI_DOUBLE,     0, 3, MPI_COMM_WORLD, &request[3]);
        MPI_Irecv(matB_recv,  slave_data, MPI_DOUBLE,     0, 4, MPI_COMM_WORLD, &request[4]);
        MPI_Irecv(matAB_recv,  slave_data, MPI_DOUBLE,     0, 5, MPI_COMM_WORLD, &request[5]);
