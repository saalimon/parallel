...
int main(int argc,char** argv)
{
    ...
    for(i=0;i<K+1;i++){
        ...
        //     if(l != id){
        //         int size = id == 0 ? data_chunk+mod_part:data_chunk;
        //         MPI_Isend(&x[start], size, MPI_DOUBLE, l, 11, MPI_COMM_WORLD,&request1[l]);
        //         MPI_Isend(&y[start], size, MPI_DOUBLE, l, 12, MPI_COMM_WORLD,&request2[l]);
        //     }
        // }
        // for(int l = 0;l<p;l++){
        //     if(l != id){
        ...
        //     fprintf(fp,"---\n");
        // }
        #pragma omp parallel for private( ftemp, fx, fy,k)
            for(j=start;j<end;j++){
                fx = 0;
                fy = 0;
                if(abs(x[j])<1000000&&abs(y[j])<1000000)
