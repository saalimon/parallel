...
void freeall();
float matA[8000][8000];
float matB[8000][8000];
float matBt[8000][8000];
float matAB[8000][8000];
int *displs;
int *scounts;
int main(int argc,char** argv)
{
    ...
    MPI_Bcast(&matBt[0][0],data.rowB*data.colB,MPI_FLOAT,0,MPI_COMM_WORLD);
    if(id==0){
        startTime = MPI_Wtime();
        avgrow = data.rowA/p;
        extra = data.rowA%p;
        for(i=1;i<p;i++){
            MPI_Send(&matA[avgrow*i+extra][0], avgrow*data.colA,MPI_FLOAT,i,3,MPI_COMM_WORLD);            
        }
        for(i=1;i<p;i++){
            MPI_Recv(&matAB[avgrow*i+extra][0], avgrow*data.colB,MPI_FLOAT,i,6,MPI_COMM_WORLD,&status);
        }
        ...
        extra = data.rowA%p;
        offset = 0;
        MPI_Recv(&matA[0][0], avgrow*data.colA,MPI_FLOAT,0,3,MPI_COMM_WORLD,&status);
        for ( i = 0; i < avgrow; i++)
        {
            for ( j = 0; j < data.colB; j++)
            {
