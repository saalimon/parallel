...
int main(int argc,char** argv)
{
    ...
    matAB =(float*)  malloc(data.rowA*data.colB*sizeof(float*));
    if(id == 0){
        startTime = MPI_Wtime();
        fp = fopen(argv[1], "r");
        if (fp) {
    ...
    // for (i=0; i<p; ++i) { 
    //     displs[i] = offset; 
    //     scounts[i] = avgrow*data.colA;
    //     offset+=avgrow*data.colA; 
    // } 
    float matA_buf[avgrow*data.colA] ;
    float matC_buf[avgrow*data.colB] ;
    MPI_Scatter(&matA[extra], avgrow*data.colA, MPI_FLOAT, matA_buf, avgrow*data.colA, MPI_FLOAT, 0, MPI_COMM_WORLD); 
    // MPI_Scatterv(&matA[extra], scounts, displs, MPI_FLOAT, matA_buf, avgrow*data.colA, MPI_FLOAT,0,MPI_COMM_WORLD); 
    MPI_Bcast(&matBt[0],data.rowB*data.colB,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    if(id!=0){
        avgrow = data.rowA/p;
        extra = data.rowA%p;
        ...
        for(i=0;i<avgrow;i++){
            for(j=0;j<data.colB;j++){
                matC_buf[i * avgrow + j] = 0;
                for(k=0;k<data.rowB;k++){
                    matC_buf[i * avgrow + j] += matA_buf[i * data.rowB + k] * matBt[k + j * data.rowB];
                }
                printf("%.1f ",sum);
            }
        } 
    }
    MPI_Gather( matC_buf, avgrow*data.colB, MPI_FLOAT, &matAB[data.rowA%p], avgrow*data.colB, MPI_FLOAT, 0, MPI_COMM_WORLD); 
    if(id==0){
        avgrow = data.rowA/p;
        extra  = data.rowA%p;
        for ( i = 0; i < avgrow+extra; ++i)
        {
            for ( j = 0; j < data.colB; ++j)
            {
                for ( k = 0; k < data.rowB; ++k)
                {
                    matAB[i * (avgrow+extra) + j] += matA[i * data.rowB + k] * matBt[k + j * data.rowB];
                }
            }
        }
        endTime = MPI_Wtime();
        ...
        if (fp) {
            ...
            for(i = 0 ; i < data.rowA ; i++){
                for(j = 0 ; j< data.colB ; j++){
                    fprintf(fp,"%.1f ",matAB[i*data.rowA+j]);
                }
                fprintf(fp,"\n");
            }
            fclose(fp);
